...
- The per-language resolver seam the spec promised exists. Adding a fifth language is **about eight files**, not the "directory, never editing core" the spec originally claimed:
  - **Required:** a **complete registry entry** (extensions, grammar, query directory, resolver key, export rule, enclosing-symbol node types and entry-point basenames — all required fields, so none can be silently omitted); **three query files** (`symbols.scm`, `imports.scm`, `calls.scm`; there is no `exports.scm`); **a resolver** in `src/resolve` plus its line in that directory's `index.ts`, or reuse of an existing `resolverId`; and **a row in `tests/languages.test.ts`**, whose `covers every registry entry` assertion goes red without one.
  - **Required when the language's visibility rule is new:** a new `ExportRule` member AND its case in `isExported` in `src/parser/parser.ts`. This is NOT optional — the switch is exhaustive with no `default`, so adding the member alone fails to compile with `TS2366` at `src/parser/parser.ts(217,66)`. That is deliberate: the same omission used to return `exported: false` for every symbol in the new language, silently, costing it every cross-file call edge.
- Beyond that, a language may need one or more OPTIONAL, shape-dependent hooks, and the plan says so rather than pretending otherwise: `METHOD_CONTAINER_TYPES` and `ENCLOSING_CLASS_TYPES` in `src/parser/parser.ts`, `SAME_PACKAGE_LANGS` in `src/indexer/same-package.ts` with its matching incremental widening, and an `@member` capture in `imports.scm`. Each of these degrades one observable thing when omitted; none of them fails silently. Spec §5.2 carries the same lists.
