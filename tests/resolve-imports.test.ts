...
describe('resolveImport', () => {
  it('resolves a relative sibling by probing extensions', () => {
    expect(resolveImport('src/services/order.ts', './notify', KNOWN, REPO_ROOT))
      .toEqual({ path: 'src/services/notify.ts', confidence: 'resolved' })
  })

  it('resolves a parent-directory specifier', () => {
    expect(resolveImport('src/services/order.ts', '../helper', KNOWN, REPO_ROOT))
      .toEqual({ path: 'src/helper.ts', confidence: 'resolved' })
  })

  it('resolves a directory to its index file', () => {
    expect(resolveImport('src/index.ts', './widgets', KNOWN, REPO_ROOT))
      .toEqual({ path: 'src/widgets/index.ts', confidence: 'resolved' })
  })

  it('resolves an explicit .js specifier to the .ts source', () => {
    expect(resolveImport('src/index.ts', './helper.js', KNOWN, REPO_ROOT).path).toBe('src/helper.ts')
  })

  it('prefers the .ts source even when both .js and .ts are indexed', () => {
    expect(resolveImport('src/index.ts', './helper.js', KNOWN, REPO_ROOT))
      .toEqual({ path: 'src/helper.ts', confidence: 'resolved' })
  })

  it('leaves bare package specifiers unresolved', () => {
    expect(resolveImport('src/index.ts', 'react', KNOWN, REPO_ROOT))
      .toEqual({ path: null, confidence: 'unresolved' })
    expect(resolveImport('src/index.ts', 'node:fs', KNOWN, REPO_ROOT))
      .toEqual({ path: null, confidence: 'unresolved' })
  })

  it('leaves a relative specifier pointing nowhere unresolved', () => {
    expect(resolveImport('src/index.ts', './missing', KNOWN, REPO_ROOT))
      .toEqual({ path: null, confidence: 'unresolved' })
  })

  it('never resolves outside the set of indexed files', () => {
    expect(resolveImport('src/index.ts', '../../../etc/passwd', KNOWN, REPO_ROOT).path).toBeNull()
  })
})
